 ...
 *
 * Hint!
 * Some further modifications there made by Stefan Galinski to fix some current bugs inside the code base.
 */
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
	...
	onKey: function(f, e) {
		if (e.getKey() === e.ENTER) {
			this.stopEditing.defer(200, this, [true]);
			e.stopPropagation();
		}
	},
	...
	onRowClick: function(g, rowIndex, e) {
		// Don't count clicks on action column items
		// solution by Condor (http://www.sencha.com/forum/showthread.php?119164-Roweditor-action-column)
	...
	getTargetColumnIndex: function(pt) {
		var grid = this.grid,
				...
				c,
				match = false;
		for (len = cms.length,c; c = cms[i]; i++) {
			if (!c.hidden) {
	...
	showTooltip: function(msg) {
		...
		if (top + h >= scroll) {
			t.initTarget(this.grid.getView().getRow(this.rowIndex));
			if (!t.rendered) {
				t.show();
				t.hide();
			}
